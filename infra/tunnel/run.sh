#!/usr/bin/env sh
# Run both Cloudflare tunnels in parallel. Exits when either child exits so
...
trap 'kill $SHOP_PID $AD_PID 2>/dev/null || true' INT TERM EXIT

# Exit when the first tunnel dies — treat one-down as fatal so the operator sees it.
wait -n
STATUS=$?
kill $SHOP_PID $AD_PID 2>/dev/null || true
exit $STATUS
